...
import (
	...
	option "github.com/hongyuxuan/dolphinscheduler-sdk-go/core/option"
...
func (p *ProcessDefinition) BatchExport(ctx context.Context, codes string) (resp []byte, err error) {
	var r interface{}
	if err = p.httpclient.Post(fmt.Sprintf("/projects/%d/process-definition/batch-export", *p.projectCode)).SetFormData(map[string]string{
		"codes": codes,
	}).SetSuccessResult(&r).Do(ctx).Err; err != nil {
		return
	}
